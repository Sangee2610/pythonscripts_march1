...
cur.execute("""
DROP TABLE IF EXISTS prd_Landing_contactkey_id;
CREATE TABLE prd_Landing_contactkey_id (
    index text,
    AccountKey text,
    accountid text,
    Id text,
...
accsample = '/home/baadmin/NCT_ETL/input_files/pg_extract_prd/ContactId_sf.csv'
...
def data_cleaning_loading(filename):
    ...
    g.close()
    g = open(new_filename)
    cur.copy_from(g, 'prd_Landing_contactkey_id',  sep=",")
    conn.commit()
    g.close()
    f.close()
