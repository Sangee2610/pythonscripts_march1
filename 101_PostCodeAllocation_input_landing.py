...
cur.execute("""
DROP TABLE IF EXISTS Landing_PostcodeAllocation;
CREATE TABLE Landing_PostcodeAllocation(
    AccountKey	 text,
    Branch text,
    Postalcode text
    )
...
input_file = '/home/baadmin/NCT_ETL/input_files/PostcodeAllocation.csv'
...
def data_cleaning_loading(filename):
    ...
    g.close()
    g = open(new_filename)
    cur.copy_from(g, 'Landing_PostcodeAllocation',  sep=",")
    conn.commit()
    g.close()
    f.close()
